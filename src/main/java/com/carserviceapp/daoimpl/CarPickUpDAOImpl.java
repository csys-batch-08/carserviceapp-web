...
public class CarPickUpDAOImpl implements CarPickUpDAO {
	...
	public List<CarPickUp> pickupview() {
		String showQuery = "select pickup_id,user_id,cus_name,cus_contact,pick_address,center_id,status from pickup order by pickup_id";
		ResultSet rs = null;
		Connection con = null;
		PreparedStatement stmt = null;
		...
		try {
			...
			while (rs.next()) {
				CarPickUp pickup = new CarPickUp(rs.getInt("pickup_id"), rs.getInt("user_id"), rs.getString("cus_name"),
						rs.getLong("cus_contact"), rs.getString("pick_address"), rs.getInt("center_id"),
						rs.getString("status"));
